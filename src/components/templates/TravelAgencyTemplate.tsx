    ...
    <div style={{ backgroundColor: `hsl(${c.background})`, color: `hsl(${c.text})` }}>
      ...
      <section className="relative overflow-hidden">
        <div
          ...
          style={{
            backgroundImage: `radial-gradient(circle at 20% 50%, hsl(${c.primary}), transparent 50%), radial-gradient(circle at 80% 50%, hsl(${c.accent}), transparent 50%)`,
          }}
        />
            ...
            <div>
              {config.content.heroBadge && (
                <Badge
                  ...
                  style={{ backgroundColor: `hsl(${c.primary} / 0.1)`, color: `hsl(${c.primary})` }}
                >
                  {config.content.heroBadge}
                </Badge>
              )}
              ...
              {config.socialLinks && (
                <div className="flex items-center gap-3 mt-6">
                  ...
                  {config.socialLinks.facebook && <a href={config.socialLinks.facebook} target="_blank" rel="noreferrer" className="opacity-50 hover:opacity-100 transition-opacity"><Facebook className="h-5 w-5" /></a>}
                  {config.socialLinks.instagram && <a href={config.socialLinks.instagram} target="_blank" rel="noreferrer" className="opacity-50 hover:opacity-100 transition-opacity"><Instagram className="h-5 w-5" /></a>}
                  {config.socialLinks.youtube && <a href={config.socialLinks.youtube} target="_blank" rel="noreferrer" className="opacity-50 hover:opacity-100 transition-opacity"><Youtube className="h-5 w-5" /></a>}
                </div>
              )}
            </div>
            <div className="relative">
              {config.content.heroImage ? (
                ...
                <div className="rounded-2xl h-[350px] md:h-[450px] flex items-center justify-center" style={{ background: `linear-gradient(135deg, hsl(${c.primary} / 0.15), hsl(${c.accent} / 0.15))` }}>
                  <Plane className="h-24 w-24" style={{ color: `hsl(${c.primary} / 0.3)` }} />
                </div>
              )}
              {/* Stats floating card */}
              ...
              return (
                <Card key={i} className="text-center hover:shadow-lg transition-all hover:-translate-y-1 border-none shadow-md">
                  <CardContent className="pt-8 pb-6">
                    <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center" style={{ backgroundColor: `hsl(${c.primary} / 0.1)` }}>
      ...
      {featured.length > 0 && (
        <section className="py-20" style={{ backgroundColor: `hsl(${c.secondary})` }}>
          <div className="container mx-auto px-4">
            <div className="text-center mb-12">
              <span className="text-sm font-semibold uppercase tracking-wider" style={{ color: `hsl(${c.primary})` }}>Popular Packages</span>
              ...
              {featured.map((pkg) => (
                <Card key={pkg.id} className="overflow-hidden hover:shadow-xl transition-all hover:-translate-y-1">
                  <div className="h-48 flex items-center justify-center relative" style={{ background: `linear-gradient(135deg, hsl(${c.primary} / 0.1), hsl(${c.accent} / 0.1))` }}>
                    ...
                    <Badge className="absolute top-3 left-3" style={{ backgroundColor: `hsl(${c.primary})`, color: "white" }}>{pkg.type}</Badge>
                  </div>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {config.content.whyChooseUs.map((item, i) => (
                <div key={i} className="flex gap-4 p-5 rounded-xl transition-colors" style={{ backgroundColor: `hsl(${c.primary} / 0.03)` }}>
            ...
            <div className="space-y-3">
              {config.content.faq.map((item, i) => (
                <div key={i} className="bg-white rounded-xl shadow-sm overflow-hidden">
                  <button
                    className="w-full text-left p-5 font-semibold flex justify-between items-center gap-4 hover:bg-gray-50 transition-colors"
      ...
      {config.content.ctaTitle && (
        <section className="py-20" style={{ background: `linear-gradient(135deg, hsl(${c.primary}), hsl(${c.primary} / 0.8))` }}>
          <div className="container mx-auto px-4 text-center text-white">
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              <Link to="/site/packages">
                <Button size="lg" className="bg-white hover:bg-gray-100 gap-2" style={{ color: `hsl(${c.primary})` }}>
